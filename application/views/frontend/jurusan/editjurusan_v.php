...
<script src="<?php echo base_url(); ?>assets/js/sweetalert2.min.js"></script>
	...
	<script>
		<?php
		foreach ($dtuser as $u) {
			$vjurusanid 	  = trim($u->jurusan_id);
			$vjurusan 		  = trim($u->jurusan_nama);
		}		
	?>
	  var valid = false;
		$(document).ready(function(){
			$("#xjurusan").keyup(function(){//change
				$("#pesan").html("check jurusan...");
				var xjurusan = $("#xjurusan").val();
				str = xjurusan;
			...
			$.ajax({
				type: "POST",
				url: "<?php echo base_url();?>os/jurusan/check_jurusan",
				data: $('#frm_tambah').serialize(),
				dataType: "html",
				success: function(txt) {
						if(txt == 1){
							$('#pesan').show(); 
							$("#pesan").html("<span style='color:red;'>Nama jurusan tersebut sudah ada..! </b><span>");
							document.getElementById("submit1").disabled = true;
							valid = false;
						} else if(txt == 0){
							if (str.length<=3) {
								$('#pesan').show(); 
								$("#pesan").html("<span style='color:red;'>Nama jurusan tidak boleh kurang dari 5 huruf..!<span>");
								document.getElementById("submit1").disabled = true;
								valid = false;
							} else {	
								$('#pesan').show(); 
								$("#pesan").html("<span style='color:green;'>Nama jurusan belum ada.<span>");
			...
			<div class="tab-content">
				...
				<div class="panel panel-flat">
					...
					<div class="panel-heading">
						...
						<h6 class="panel-title"><b>EDIT JURUSAN</b></h6>
					</div>
					
					<div class="panel-body">
						<form id="frm_tambah" action="<?php echo base_url('os/jurusan/xupdate');?>" method="post" enctype="multipart/form-data">
							<input type="hidden" name="xid" value="<?=$vjurusanid;?>">
							<div class="form-group">
								<div class="row">
									<div class="col-md-6">
										<label>Nama Jurusan <span class="text-danger">*</span></label>
										<input type="text" style="font-size: 14px;" name="xjurusan" id="xjurusan" class="form-control  text-semibold required" autocomplete="off" minlength="5" maxlength="50" placeholder="min 5 character" value="<?=$vjurusan;?>" required>
										<div id="pesan"></div>
									</div>
	                  ...
	                  <div class="text-right">
	                  	...
	                  	<a href="<?php echo base_url('os/jurusan');?>"><button type="button" class="btn btn-danger btn-labeled btn-rounded"><b><i class="icon-cross2"></i></b> Batal</button></a>
	                  </div>
						</form>
				</div>
			</div>
